...
getwd()
setwd("E:/drug_repurposing/GSE_datesets_results/finished_R_DEG/GSE210682/")
...
eSet <- getGEO("GSE210682", destdir = ".", getGPL = T) 

file_path <- "./GSE210682_raw_counts_GRCh38.p13_NCBI.tsv"
...
gtf_data <- read_tsv("./Human.GRCh38.p13.annot.tsv")


gene_info <- data.frame(
  gene_id = gtf_data$GeneID,
  gene_symbol = gtf_data$Symbol
)
...
colnames(gene_info) <- c("GeneID", "HUGOSYMBOL")



exprSet <- merge(exprSet, gene_info, by = "GeneID", all.x = TRUE) 
...
for (i in 1:nrow(pdata)) {
  ...
  if (grepl('Control', pdata$characteristics_ch1.2[i])) {
