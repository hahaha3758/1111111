...
getwd()
setwd("E:/drug_repurposing/GSE_datesets_results/finished_R_DEG/GSE194368_result/")
...
eSet <- getGEO("GSE194368", destdir = ".", getGPL = T) 
file_path <- "./GSE194368_OUD_rawcounts.csv"
...
colnames(hugo_symbols) <- c("X", "HUGOSYMBOL")

exprSet <- merge(exprSet, hugo_symbols, by = "X", all.x = TRUE)
...
write.csv(filtered_DEG_result, file = './DESeq2_result/DEGs_DESeq2.csv', row.names = TRUE, fileEncoding = "UTF-8")
